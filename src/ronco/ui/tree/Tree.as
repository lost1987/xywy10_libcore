package ronco.ui.tree {
	...
	import ronco.ui.tree.TreeCellRenderer;
	import ronco.ui.tree.TreeCellRendererVO;
	import ronco.ui.tree.TreeEvent;
	 ...
	 *      <node label="level1">
	 * 		<node label="level1" labelColor="ffffff" hasIcon="true"/>
	 *
	...
	public class Tree extends UIElement {
		...
		public var iconOpen:Bitmap;
		public var iconClose:Bitmap;
		public var imgSelected:Bitmap;
		...
		public var logicHeight:int = 0;
		
		/**
		 *Array<TreeCellRendererVO>
		 */
		protected var treeCellRendererVOArrayProvider:Array = new Array();
		/**
		 *Array<TreeCellRenderer> 递归存放
		 */
		protected var treeCellRendererArray:Array = new Array();
		/**
		 *顺序存放的所有 TreeCellRenderer
		 */
		protected var treeCellRendererList:Array = new Array();
...
//		}
		...
		public function Tree(_name:String, _parent:UIElement)
		{
			super(UIDef.UI_TREE, _name, _parent);
			this.init();
		}
		...
		protected function drawLayout():void {
			...
			for (var i:int = 0; i < this.treeCellRendererArray.length; i++) {
				...
				logicHeight = childDis.y + childDis.height;
			}
		}
		...
		protected function resolveXMLData(dataXML:XML):void {
			...
			var len:int = treeCellRendererVOArrayProvider.length;
			var i:int = 0;
			
			while(i < len)
			{
				treeCellRendererVOArrayProvider.pop();
				
				++i;
			}
			//treeCellRendererVOArrayProvider.splice(0, treeCellRendererVOArrayProvider.length);
		...
		protected function resolveArrayData(arrayTreeCellRendererVO:Array):void {
			...
			var len:int = treeCellRendererArray.length;
			var i:int = 0;
			
			while(i < len)
			{
				treeCellRendererArray.pop();
				
				++i;
			}
			
			len = treeCellRendererList.length;
			i = 0;
			
			while(i < len)
			{
				treeCellRendererList.pop();
				
				++i;
			}			
			
			//this.treeCellRendererArray.splice(0, treeCellRendererArray.length);
			//this.treeCellRendererList.splice(0, treeCellRendererList.length);
		...
		protected function splitVOToRenderer(level:int, parentRenderer:TreeCellRenderer, data:TreeCellRendererVO):TreeCellRenderer {
			...
			if (this._itemRenderer == null) {
				treeCellRenderer = new TreeCellRenderer(this);
			} else {
